...
class MROSolver {
    ...
    void solveItems() {
        #pragma omp parallel for schedule(dynamic) num_threads(m_k)
...
public:
    MROSolver(Graph * graph, int a, int k)
        : m_graph(graph), m_a(a), m_k(k) {
            m_minimum.m_value = m_graph->weightSum;
            m_minimum.m_bits = vector<bool>(m_graph->numberOfNodes, 0);
            m_dfsMaxDepth = m_graph->numberOfNodes > 3 ? 3 : m_graph->numberOfNodes-1;
        }
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 4) {
        cout << "Not enough arguments! ./a.out FILENAME PARAMETER_A NUMBER_OF_THREADS" << endl;
        return 1;
    }
    ...
    MROSolver * solver = new MROSolver(graph, a, k);
